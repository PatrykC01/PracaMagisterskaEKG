...
OUTPUT_DIR = "/kaggle/working/architecture_sweep_1d_2d_stft_65x110"
...
N_TRIALS_TARGET = 20
MAX_TRIAL_ATTEMPTS_PER_MODEL = 80
...
def restore_databases_if_requested():
    ...
    for model_id in MODELS_TO_RUN:
        ...
        if os.path.exists(destination):
            continue
        source = find_file_recursive(RESUME_INPUT_DIR, filename)
...
def save_manifest(studies_info, elapsed_hours):
    payload = {
        "protocol": "equal-budget architecture sweep in RAW 1D and STFT 2D",
        "raw_window": {"before": 65, "after": 110, "length": 175},
