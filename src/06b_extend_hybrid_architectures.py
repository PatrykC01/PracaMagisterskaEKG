...
OUTPUT_DIR = "/kaggle/working/architecture_sweep_hybrid_extension_65x110"

RESUME_INPUT_DIR = "/kaggle/input"
PREVIOUS_LIVE_OUTPUT_DIR = "/kaggle/working/architecture_sweep_1d_2d_stft_65x110"
...
N_TRIALS_TARGET = 20
MAX_TRIAL_ATTEMPTS_PER_MODEL = 160
MAX_RUNTIME_HOURS = 10.5

MIN_EXPECTED_EXISTING_CNN2D_COMPLETE = 16
...
def save_study_outputs(study, model_id):
    ...
    with open(os.path.join(OUTPUT_DIR, f"pareto_{suffix}.json"), "w", encoding="utf-8") as file:
        json.dump(
            [trial_payload(trial, model_id, "pareto") for trial in pareto_trials],
            file,
            ensure_ascii=False,
            indent=2,
        )
    ...
    rare_trial = max(pareto_trials, key=lambda trial: (trial.values[0], trial.values[1]))
    balanced_trial = max(
        pareto_trials,
        key=lambda trial: (
    ...
    with open(
        ...
        json.dump(trial_payload(rare_trial, model_id, "rare_priority"), file, ensure_ascii=False, indent=2)
    with open(
        os.path.join(OUTPUT_DIR, f"candidate_balanced_{suffix}.json"),
        "w", encoding="utf-8",
    ) as file:
        json.dump(
            trial_payload(balanced_trial, model_id, "balanced_min_rare"),
            file, ensure_ascii=False, indent=2,
        )

    return int(rare_trial.number), int(balanced_trial.number)
...
def restore_databases_if_requested():
    os.makedirs(OUTPUT_DIR, exist_ok=True)

    search_roots = []
    for root_dir in [PREVIOUS_LIVE_OUTPUT_DIR, RESUME_INPUT_DIR]:
    ...
    for model_id in MODELS_TO_RUN:
        ...
        for root_dir in search_roots:
            source = find_file_recursive(root_dir, filename)
...
def validate_required_cnn2d_database():
    ...
    if "CNN2D" not in MODELS_TO_RUN:
        return

    db_path = os.path.join(OUTPUT_DIR, "study_cnn2d_multi_v1.db")
    if not os.path.exists(db_path):
        raise FileNotFoundError(
            "Nie znaleziono study_cnn2d_multi_v1.db. Podlacz jako Kaggle Input "
            "wyniki poprzedniego sweepu architektur. Skrypt celowo nie tworzy "
            "nowego badania CNN2D od zera."
        )
    ...
    try:
        ...
        raise RuntimeError(
            "Znaleziono plik study_cnn2d_multi_v1.db, ale nie zawiera on "
            "oczekiwanego badania cnn2d_65x110_multi_v1."
        ) from error

    n_complete = sum(
        trial.state == optuna.trial.TrialState.COMPLETE for trial in study.trials
    )
    if n_complete < MIN_EXPECTED_EXISTING_CNN2D_COMPLETE:
        raise RuntimeError(
            f"Baza CNN2D ma tylko {n_complete} COMPLETE; oczekiwano co najmniej "
            f"{MIN_EXPECTED_EXISTING_CNN2D_COMPLETE}. To prawdopodobnie nie jest "
            "baza z poprzedniego sweepu."
        )


def save_manifest(studies_info, elapsed_hours):
    payload = {
        "protocol": "CNN2D completion plus equal-budget CNN-BiLSTM extension",
        "raw_window": {"before": 65, "after": 110, "length": 175},
        ...
        "trials_target_per_model": N_TRIALS_TARGET,
        "max_trial_attempts_per_model": MAX_TRIAL_ATTEMPTS_PER_MODEL,
        "screen_seeds": SWEEP_SEEDS,
    ...
    with open(
        os.path.join(OUTPUT_DIR, "architecture_sweep_manifest.json"),
        "w", encoding="utf-8",
    ) as file:
...
def main():
    invalid = set(MODELS_TO_RUN) - set(MODEL_DOMAIN)
    if invalid:
        raise ValueError(f"Nieznane modele: {sorted(invalid)}")
    if not MODELS_TO_RUN:
        raise ValueError("MODELS_TO_RUN nie moze byc puste.")

    os.makedirs(OUTPUT_DIR, exist_ok=True)
    restore_databases_if_requested()
    ...
    image_train_dataset = None
    image_val_dataset = None
    if any(MODEL_DOMAIN[model_id] == "STFT_2D" for model_id in MODELS_TO_RUN):
    ...
    for model_id in MODELS_TO_RUN:
        ...
        suffix = model_id.lower()
        db_path = os.path.join(OUTPUT_DIR, f"study_{suffix}_multi_v1.db")
        study = optuna.create_study(
            study_name=f"{suffix}_65x110_multi_v1",
            storage=f"sqlite:///{db_path}",
            sampler=optuna.samplers.NSGAIISampler(seed=20260810),
        ...
        while (
            existing_complete < N_TRIALS_TARGET
            and len(study.trials) < MAX_TRIAL_ATTEMPTS_PER_MODEL
            and time.time() < deadline
        ):
            study.optimize(
                make_objective(model_id, train_dataset, val_dataset),
                n_trials=1,
        ...
        rare_trial, balanced_trial = save_study_outputs(study, model_id)
        n_complete = sum(
            trial.state == optuna.trial.TrialState.COMPLETE for trial in study.trials
        )
        n_pruned = sum(
        ...
        studies_info[model_id] = {
            "domain": domain,
            "database": os.path.basename(db_path),
            "n_trials_total": len(study.trials),
            "n_trials_complete": int(n_complete),
            "n_trials_pruned": int(n_pruned),
            "rare_priority_trial": rare_trial,
            "balanced_trial": balanced_trial,
            "pareto_trial_numbers": [int(trial.number) for trial in study.best_trials],
        }

        save_manifest(studies_info, (time.time() - session_start) / 3600.0)

    elapsed_hours = (time.time() - session_start) / 3600.0
    save_manifest(studies_info, elapsed_hours)
